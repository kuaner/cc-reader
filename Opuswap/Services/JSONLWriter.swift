...
enum JSONLWriter {
    static func backup(url: URL) throws {
    ...
    static func deleteMessages(uuids: [String], from url: URL) throws {
    ...
    static func restore(url: URL) throws {
        let fileManager = FileManager.default
        let backupUrl = url.deletingPathExtension().appendingPathExtension("bak")
    ...
    static func updateMessageContent(uuid: String, newContent: String, in url: URL) throws {
        let content = try String(contentsOf: url, encoding: .utf8)
        let lines = content.components(separatedBy: .newlines)
    ...
    static func deleteMessagesAfter(uuid: String, from url: URL, inclusive: Bool = false) throws {
        let content = try String(contentsOf: url, encoding: .utf8)
        let lines = content.components(separatedBy: .newlines)
        var filteredLines: [String] = []
