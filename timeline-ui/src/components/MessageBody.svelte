<script lang="ts">
  ...
  import type { MessageBodyOptions, MarkdownTone } from '../lib/messageBodyOptions';
  ...
  const emptyClass = $derived(
    ['whitespace-pre-wrap break-words', tone === 'user' ? 'text-[color:var(--surface-user-text)]' : '']
      .filter(Boolean)
      .join(' '),
  );
</script>
...
{:else}
  <div class={proseForTone(tone)} data-markdown-base64={encoded}>
    <div class="whitespace-pre-wrap break-words">{@html fallbackHtml}</div>
  </div>
{/if}
