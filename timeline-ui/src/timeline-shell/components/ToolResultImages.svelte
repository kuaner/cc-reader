<script lang="ts">
  ...
  const images = $derived(
    Array.isArray(payload.resultImages) ? payload.resultImages : [],
  );
...
{#if items.length > 0}
  <div class="mt-2 flex flex-col gap-2">
    {#each items as item (String(item?.base64 || "").slice(0, 24))}
      ...
      <div
        class="flex h-[220px] w-[min(360px,100%)] items-center justify-center overflow-hidden rounded-[10px] border border-(--border) bg-(--attachment-bg)"
      >
        <img
