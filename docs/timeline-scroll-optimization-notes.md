...
### 7. Resize 滚动位置保持（2026-04-02）

Sidebar 开关 / 分屏面板增减会触发 WebView 的 `resize` 事件，内容重排导致 `scrollHeight` 变化。

**问题**：之前的实现用 100ms debounce + `scrollTo(bottom)`，但在 250ms sidebar 动画过程中 scrollHeight 持续变化，产生 "滚动条跳到中间→再快速回到底部" 的视觉抖动。

**解决方案**：在 resize burst 的**第一个事件**就捕获 `followingBottom` 状态（此时还未重排，状态准确），然后用 `requestAnimationFrame` 循环每帧执行 `scrollTo(bottom, instant)` 保持钉在底部。300ms debounce 结束（动画完成后）停止 rAF 循环。

```javascript
// 核心逻辑
window.addEventListener('resize', function () {
  if (!resizeTimer) {
    wasFollowing = scrollState.followingBottom;  // 捕获重排前的状态
    if (wasFollowing && !rafId) {
      rafId = requestAnimationFrame(pinToBottom);  // 每帧钉底
    }
  }
